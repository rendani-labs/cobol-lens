...
- Expand Copybooks (Preview): new command `COBOL Lens: Expand Copybooks (Preview)` (Command Palette and editor context menu) that opens a read-only virtual document with every `COPY` expanded inline, including nested copybooks and `COPY ... REPLACING` (pseudo-text substitution). Unresolved or recursive copies are left in place with a note, and code that precedes a `COPY` on the same line is preserved. No compiler required.
- Embedded SQL/CICS support: `EXEC SQL`/`EXEC CICS`/`EXEC DLI ... END-EXEC` blocks that span multiple lines are now foldable, and hovering a host variable (a name prefixed with `:`) shows a "Host variable" note in addition to the resolved COBOL field. Go to Definition, Find All References and Highlight Occurrences already work on host variables (the leading `:` is not part of the name).

### Fixed
- Field size calculation (hover, inlay hints, Record Layout): a `USAGE` keyword that appeared only as part of a data name (for example `WS-ANNO-COMP`, `WS-BINARY-FLAG`, `WS-INDEX-KEY`) was wrongly interpreted as the actual usage, giving incorrect byte sizes (e.g. `PIC 9(004)` counted as 2 bytes instead of 4). Usage keywords (`COMP`, `COMP-1..5`, `COMPUTATIONAL*`, `BINARY`, `PACKED-DECIMAL`, `INDEX`, `POINTER`) are now recognized only when they are standalone tokens, not when they are a hyphenated part of an item name. This also corrects the size of groups and `REDEFINES` areas that contain such fields.
